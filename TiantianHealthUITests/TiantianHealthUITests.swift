...
final class TiantianHealthUITests: XCTestCase {
    ...
    func testCoreMVPFlow() throws {
        ...
        app.buttons["关闭"].tap()
        app.buttons["继续"].tap()
        XCTAssertTrue(app.staticTexts["设置备用活动基准"].waitForExistence(timeout: 3))
        ...
        XCTAssertTrue(app.buttons["新建食物"].waitForExistence(timeout: 4))
        XCTAssertTrue(app.segmentedControls["food-meal-picker"].exists)
        app.segmentedControls["food-meal-picker"].buttons["午餐"].tap()
        capture("05a-food-meal-picker")
        ...
        XCTAssertEqual(addToMeal.label, "加入午餐")
        addToMeal.tap()
        ...
        todayBudgetDay.tap()
        XCTAssertTrue(app.staticTexts["目标热量缺口"].waitForExistence(timeout: 4))
        XCTAssertTrue(app.staticTexts["实际摄入"].exists)
    ...
    func testPhotoAnalysisResultLayoutAndDuplicateResolution() throws {
        ...
        XCTAssertTrue(app.buttons["photo-analysis-history"].exists)
